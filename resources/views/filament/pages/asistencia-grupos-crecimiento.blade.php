    ...
    @if (filled($this->grupo_id))
        <div class="mt-6 rounded-3xl border border-gray-200 bg-white p-5 shadow-sm">
            <div class="flex items-center justify-between gap-4">
                <div>
                    <h2 class="text-lg font-semibold text-gray-900">Integrantes activos</h2>
                    <p class="text-sm text-gray-500">Puedes quitar una persona del grupo sin perder el historial de asistencias.</p>
                </div>
            ...
            @if ($integrantesActivos->isEmpty())
                <div class="mt-4 rounded-2xl border border-dashed border-gray-300 bg-gray-50 p-6 text-sm text-gray-500">
                ...
                <div class="mt-4 grid gap-3 md:grid-cols-2">
                    @foreach ($integrantesActivos as $integrante)
                        <div class="flex items-center justify-between gap-3 rounded-2xl bg-gray-50 px-4 py-3">
                            <div class="font-medium text-gray-900">{{ $integrante['label'] }}</div>
