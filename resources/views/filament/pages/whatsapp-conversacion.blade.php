<x-filament-panels::page>
    ...
    <div class="space-y-6">
        <div class="rounded-3xl border border-gray-200 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-white/5">
            @if (! $summary)
                <div class="rounded-2xl border border-dashed border-gray-300 bg-gray-50 p-8 text-center text-sm text-gray-500 dark:border-white/10 dark:bg-white/5 dark:text-gray-300">
            ...
            @else
                <div class="flex flex-wrap items-center gap-3">
                    ...
                    <span class="inline-flex rounded-full px-3 py-1 text-xs font-medium {{ $summary['ventana_abierta'] ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-500/15 dark:text-emerald-300' : 'bg-gray-100 text-gray-700 dark:bg-white/10 dark:text-gray-300' }}">
                ...
                <div class="mt-6 space-y-3">
                    @foreach ($messages as $message)
                        <div class="flex {{ $message->isOutbound() ? 'justify-end' : 'justify-start' }}">
                            <div class="max-w-2xl rounded-2xl px-4 py-3 {{ $message->isOutbound() ? 'bg-primary-100 text-primary-900 dark:bg-primary-500/20 dark:text-primary-100' : 'bg-gray-100 text-gray-900 dark:bg-white/10 dark:text-white' }}">
                ...
                <div class="mt-6 rounded-2xl border border-gray-200 bg-gray-50 p-4 dark:border-white/10 dark:bg-white/5">
                    {{ $this->form }}
