    ...
    <div class="space-y-6 px-4">
        <div class="grid gap-4 md:grid-cols-4">
            <div class="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-white/5">
                <div class="text-sm text-gray-500 dark:text-gray-300">Grupos</div>
                <div class="mt-2 text-2xl font-semibold text-gray-900 dark:text-white">{{ $summary['total_grupos'] }}</div>
            </div>
            <div class="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-white/5">
                <div class="text-sm text-gray-500 dark:text-gray-300">Personas únicas</div>
        ...
        <div class="rounded-3xl border border-gray-200 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-white/5">
            <div class="flex flex-wrap items-center justify-between gap-4">
                <div class="px-4">
                    <h2 class="text-lg font-semibold text-gray-900 dark:text-white">{{ $this->record->nombre }}</h2>
                    ...
                    @if (filled($this->record->descripcion))
                        <p class="mt-2 text-sm text-gray-600 dark:text-gray-300">{{ $this->record->descripcion }}</p>
                    @endif
                </div>
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-white/10">
                            @foreach ($rows as $row)
                                <tr class="align-top">
                                    ...
                                    <td class="px-4 py-4 text-gray-700 dark:text-gray-200">{{ $row['grupos_metagrupo'] }}</td>
                                    <td class="px-4 py-4">
